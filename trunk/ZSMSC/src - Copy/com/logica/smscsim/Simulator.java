...
public class Simulator
{
    ...
    protected void start() throws IOException
    {
        if (smscListener == null) {
            ...
            System.out.println("Port number : 10035");
    ...
    public boolean isExistConn(String user) {
        boolean result = false;
        if (smscListener != null) {
            System.out.println("Check connection of "+user);
            synchronized (processors) {
                ...
                for(int i=0; i<procCount; i++) {
                    ...
                    if(user.equals(proc.getSystemId())) {
                        //session.stop();
                        //System.out.print("Stopping session "+proc.getSystemId() + " ...");
                        //System.out.println(" stopped.");
                        System.out.println("Session "+proc.getSystemId() + " exists");
                        result = true;
                    }
                }
            }
     ...
     * on the standard output.
     */
    public String krxmessageList()
    {
        StringBuffer t = new StringBuffer();
        if (smscListener != null) {
            //messageStore.print();
            t = messageStore.krxprint();
        } else {
            //System.out.println("You must start listener first.");
            t.append("You must start listener first.");
        }
     ...
     * Prints all currently connected clients on the standard output.
     */
    public StringBuffer krxlistClients()
    {
        ...
        if (smscListener != null) {
            synchronized (processors) {
                ...
                if (procCount > 0) {
                    ...
                    //System.out.println("No client connected.");
                    sb.append("No client connected.");
                }
            }
        } else {
            //System.out.println("You must start listener first.");
            sb.append("You must start listener first.");
        }
    ...
    public String sendMessage(String toSMPP, String shortCode, String msisdn, String msg) throws IOException
    {
                    ...
                    if (proc.getSystemId().equals(client)) {
                        if (proc.isActive()) {
                            ...
                            try {
                                ...
                                destAddress = shortCode;
                                shortMessage = msg;
                                registeredDelivery = Byte.parseByte("1");
                            ...
                            try {
                                sms.setDestAddr(shortCode);
                                sms.setSourceAddr(msisdn);
                                sms.setShortMessage(msg);
                                sms.setEsmClass(Byte.parseByte("1"));

                                //request.setShortMessage(message);
                                //proc.serverRequest(request);
                                proc.serverRequest(sms);
                                ret = "Message sent.";
                            } catch (WrongLengthOfStringException e) {
